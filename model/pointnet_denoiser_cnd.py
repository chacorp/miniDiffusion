...
Architecture per block:
    Self-Attn(h) -> Cross-Attn(Q=h, K/V=global_token) -> FiLM(t) -> FFN

Condition pipeline:
    cond_pc [B,M,C] -> PointNetEncoder -> pre_pool [B,M,D]
                    -> LearnedPooling (FC + sum) -> global_token [B,1,D]

Aux reconstruction (improves encoder):
    pre_pool [B,M,D] -> PointNetDecoder -> recon [B,M,C]
    L_recon = MSE(recon, cond_pc)
...
CFG: replace condition with null_token for unconditional path.
"""
import torch
...
class PointNetEncoder(nn.Module):
    """Point-wise MLP encoder. Returns pre-pool per-point features.

    Args:
        in_dim:     input coordinate dimension C
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """[B, M, C] -> [B, M, D]  (pre-pool features)"""
        return self.net(x)


class LearnedPooling(nn.Module):
    """Reduces M point features to a single global token via FC + sum.

    FC projects each point feature, then sums over the M dimension.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """[B, M, D] -> [B, 1, D]"""
        return self.fc(x).sum(dim=1, keepdim=True)


class PointNetDecoder(nn.Module):
    """Point-wise MLP decoder. Reconstructs coordinates from pre-pool features.

    Mirrors PointNetEncoder structure: same depth and width, reversed dims.

    Args:
        in_dim:     input feature dimension D (= encoder out_dim)
        hidden_dim: intermediate MLP width
        out_dim:    output coordinate dimension C
    ...
    def __init__(self, in_dim: int = 256, hidden_dim: int = 128, out_dim: int = 2):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(in_dim, hidden_dim),
            nn.LayerNorm(hidden_dim),
...
class CrossAttentionBlock(nn.Module):
    ...
    forward(h, cond_token, t_emb):
        h          -> [B, N, D]   noisy point features
        cond_token -> [B, 1, D]   global condition token
    ...
    def __init__(self, dim: int, num_heads: int, t_dim: int, dropout: float = 0.0):
        ...
        # Cross-attention (K/V = 1 global token)
    ...
    def forward(
        ...
        # 2. Cross-attention: h attends to global condition token
        h = h + self.cross_attn(self.norm2(h), cond_token, cond_token,
...
class PointNetDenoiserCnd(nn.Module):
    """Point cloud denoiser conditioned on a reference point cloud.

    Condition pipeline:
        cond_pc -> PointNetEncoder -> pre_pool [B,M,D]
               -> LearnedPooling  -> global_token [B,1,D]  (cross-attn K/V)

    Aux decoder (trained jointly):
        pre_pool -> PointNetDecoder -> recon [B,M,C]

    Args:
        input_dim:  coordinate dimension C
    ...
    forward(x, t, cond_token):
        x          -> [B, N, C]   noisy input points
        t          -> [B]         timestep indices
        cond_token -> [B, 1, D]   global condition token (from encode_cond or get_null_tokens)
        out        -> [B, N, C]   predicted noise eps
    ...
    def __init__(
        ...
        # Learned pooling: [B, M, D] -> [B, 1, D]
        self.pooling = LearnedPooling(hidden_dim)

        # Aux decoder: [B, M, D] -> [B, M, C]
        self.decoder = PointNetDecoder(hidden_dim, hidden_dim // 2, input_dim)
    ...
    def encode_cond(self, cond_pc: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Encode condition point cloud.

        cond_pc      -> [B, M, C]
        global_token -> [B, 1, D]  (for cross-attn)
        pre_pool     -> [B, M, D]  (for aux decoder)
        """
        pre_pool     = self.encoder(cond_pc)       # [B, M, D]
        global_token = self.pooling(pre_pool)      # [B, 1, D]
        return global_token, pre_pool

    def decode(self, pre_pool: torch.Tensor) -> torch.Tensor:
        """Reconstruct coordinates from pre-pool features.

        pre_pool -> [B, M, D]
        out      -> [B, M, C]
        """
        return self.decoder(pre_pool)
...
class SelfReconCollate:
    ...
    from the same shape point pool.
...
def ddim_sample_cnd(
    ...
    cond_pc:    torch.Tensor,        # [B, M, C]
    n_points:   int,
    alpha_bars: torch.Tensor,        # [T]
    T:          int,
    n_steps:    int   = 50,
    w:          float = 1.5,
    t_start:    int   = None,        # None = full generation; int = SDEdit noise level
    ...
    x_start    -> [B, N, C]   clean source for SDEdit; None for generation
    ...
    # Encode condition once; reuse across all DDIM steps
    cond_token, _ = model.encode_cond(cond_pc)      # [B, 1, D]
    ...
    for i in range(n_steps):
        ...
        eps_u = model(x, t_vec, null_token)
        eps_c = model(x, t_vec, cond_token)
